...
use super::modal_button::{primary_btn, ghost_btn};
...
pub fn create_artifact_modal_view<'a>(
    title: &'a str,
    description: &'a str,
    ...
    let folder_row = row![
        button(
            text("Select Folder").size(11).font(Font::default())
    ...
    // ── Header ───────────────────────────────────────────────────────────
    let header = modal_header("New Artifact", alpha);

    // ── Actions ──────────────────────────────────────────────────────────
    let footer_row = row![
        ghost_btn("Cancel", Message::CloseCreateModal, alpha),
        Space::new().width(Length::Fill),
        primary_btn("Create Artifact", Message::SubmitCreateArtifact, alpha, is_loading),
    ]
    .spacing(0)
    .align_y(Alignment::Center)
    .width(Length::Fill);
